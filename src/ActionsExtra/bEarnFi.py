...
"""
    ...
    def initialPass():
        ...
        action1_prestate_dependency = [action1, action2, action3, action4]
        action2_prestate_dependency = [action1, action2, action3, action4]
        action3_prestate_dependency = [action1, action2, action3, action4]
...
class Withdraw(bEarnFiAction):
    ...
    def actionStr(cls):
        ...
        action += '''      Bvaultsbank.withdraw(13, $$ * * 1e18);\n'''
        return action
    ...
    def add1PointValue(cls, inputs, values):
        ...
        if values == None or len(values) != 8:
            return -1
        point = [values[0], values[1], values[2], values[3], input]
...
# Frozen
class WithdrawAll(bEarnFiAction):
    points = []
    values = [[], [], [], []]
    ...
    @classmethod
    def actionStr(cls):
        action = "      // Action: WithdrawAll\n"
        action += '''      Bvaultsbank.withdrawAll(13);\n'''
        return action


    @classmethod
    def collectorStr(cls):
        action = "      // Collect: WithdrawAll\n"
        action += '''        str1 = BvaultsSummary();
        uint BUSDgot = BUSD.balanceOf(address(this));
        (uint sharesSpent,) = Bvaultsbank.userInfo(13, address(this));

        Bvaultsbank.withdrawAll(13);
