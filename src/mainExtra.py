...
def main():
    ...
    # argv[0]
    config.benchmarkName = 'Warp'
    # argv[1]
    config.method = 1 # method 0 for nearest interpolation
    ...
    _, actions, actionWrapper, maxSynthesisLen = getActionsFromContractName()
    
    if hasattr(actionWrapper, 'runinitialPass'):
        actionWrapper.runinitialPass()
    
    if config.benchmarkName == 'bZx1' or config.benchmarkName == 'Eminence' or \
        config.benchmarkName == 'ValueDeFi' or config.benchmarkName == 'bEarnFi':
