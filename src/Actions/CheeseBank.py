...
'''
    ...
    def initialPass():
        ...
        action_list_1 = [action3, action4, action7]

        action3_prestate_dependency = [action1, action2, action8]
        action4_prestate_dependency = [action1, action2, action8, action3]
        action7_prestate_dependency = [action1, action2, action8, action4]
        # # seq of actions

        ActionWrapper = CheeseBankAction
        action_lists = [action3_prestate_dependency + [action3], action4_prestate_dependency + [action4], \
...
class RefreshCheeseBank(CheeseBankAction):
    ...
    def collectorStr(cls):
        ...
        action += '''       str1 = UniswapSummary();
        str[0] = "UNI_V2-CHEESE-ETH";
        CheesePriceOracle(CheesePriceOracleAddress).refresh(str);
        price = CheesePriceOracle(CheesePriceOracleAddress).price("UNI_V2-CHEESE-ETH") / 10 ** 5;
        revert(appendWithSpace(str1, uint2str(price)));'''
        return action
    ...
    def add1PointValue(cls, inputs, values):
        ...
        cls.points.append(point)   # Uniswap Cheese reserve
                                                 # Uniswap WETH reserve
        v0= cls.aliquotValues(values)
        cls.values[0].append(v0)  # CheeseBank Price
...
class LP2LQ(CheeseBankAction):
    ...
    @classmethod
    def collectorStr(cls):
        action = "      // Collect: LP2LQ\n"
        action += '''       price = CheesePriceOracle(CheesePriceOracleAddress).price("UNI_V2-CHEESE-ETH") / 10 ** 5;
        (,uint LQgot,) = Unitroller.getAccountLiquidity(address(this));

        cERC20(CheeseETHAddress).mint($$ * 10 ** 18);

        (,uint LQgot2,) = Unitroller.getAccountLiquidity(address(this));
        revert(appendWithSpace(uint2str(price), uint2str((LQgot2 - LQgot)/ 10 ** 18)));\n
        '''
        return action
    ...
    def add1PointValue(cls, inputs, values):
        ...
        if values == None or len(values) != 2:
            return -1
        if [values[0], inputs[-1]] in cls.points:
            return -2
        cls.points.append([values[0], inputs[-1]])   # Cheesebank Price
                                                       # LQgot
        v0 = cls.aliquotValues(values)[0]
    ...
    @classmethod
    def simulate(cls, input):
        inputs = [cls.globalStates[3], input]
        output0 = cls.approximator0(inputs)
        return output0
...
class BorrowCheese_DAI(CheeseBankAction):
    ...
    @classmethod
    def collectorStr(cls):
        action = "      // Collect: BorrowCheese_DAI\n"
        action += '''       (,uint LQcost,) = Unitroller.getAccountLiquidity(address(this));
        cERC20(CheeseDAIAddress).borrow( $$ * 10 ** 18);
        (,uint LQcost2,) = Unitroller.getAccountLiquidity(address(this));
        revert(uint2str((LQcost - LQcost2) / 10 ** 18));  '''
        return action
    ...
    @classmethod
    def add1PointValue(cls, inputs, values):
        if values == None or len(values) != 1 or values[0] == 2:
            return -1
        ...
        v0 = cls.aliquotValues(values)
        cls.values[0].append(v0)  # Warp getBorrowLimit
        cls.hasNewDataPoints = True
        return 1
...
# Test for initial pass of data collecton: 
    CheeseBankAction.initialPass()
#     CheeseBankAction.runinitialPass()
