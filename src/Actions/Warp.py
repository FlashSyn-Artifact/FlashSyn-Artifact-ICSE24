...
"""
    ...
    def initialPass():
        ...
        action4_prestate_dependency = [action2, action1, action3]
        action5_prestate_dependency = [action2, action3, action1, action4]
        action6_prestate_dependency = [action2, action3, action1, action4]
...
class LP2BorrowLimit(WarpAction):
    ...
    def collectorStr(cls):
        action = "      // Collect: LP2BorrowLimit\n"
        action += '''       str1 = UNI_V2Summary();
        warpVaultLP.provideCollateral( $$ *10**18);
    ...
    def add1PointValue(cls, inputs, values):
        ...
        if [values[0], values[1], inputs[-1]] in cls.points:
            return -2

        cls.points.append([values[0], values[1], inputs[-1]])   # Uniswap DAI reserve
...
class BorrowSC_USDC(WarpAction):
    ...
    @classmethod
    def collectorStr(cls):
        action = "      // Collect: BorrowSC_USDC\n"
        action += '''       uint BLspent = warpControl.getTotalBorrowedValue(address(this));
        uint USDCgot = USDC.balanceOf(address(this));
        
        warpControl.borrowSC(USDCAddress, $$ * 10 ** 6);

        BLspent = (warpControl.getTotalBorrowedValue(address(this)) - BLspent) / 10 ** 6;
        USDCgot = (USDC.balanceOf(address(this)) - USDCgot ) / 10 ** 6;
        revert(appendWithSpace(uint2str(BLspent), uint2str(USDCgot)));
...
'''
    ...
    def add1PointValue(cls, inputs, values):
        ...
        # input = inputs[-1]
        cls.points.append(point)   # USDC borrowed proposed

        v0, v1 = cls.aliquotValues(values)
        cls.values[0].append(v0)  # USDC Borrowed actually
        cls.values[1].append(v1)  # BL spent
        cls.hasNewDataPoints = True
        return 1
...
class BorrowSC_DAI(WarpAction):
    ...
    @classmethod
    def collectorStr(cls):
        action = "       // Collect: BorrowSC_DAI\n"
        action += '''        uint BLspent = warpControl.getTotalBorrowedValue(address(this)) / 10 ** 6;
        uint DAIgot = DAI.balanceOf(address(this));

        warpControl.borrowSC(DAIAddress, $$ * 10 ** 18);

        BLspent = (warpControl.getTotalBorrowedValue(address(this)) - BLspent) / 10 ** 6;
        DAIgot = (DAI.balanceOf(address(this)) - DAIgot ) / 10 ** 18;
        revert( appendWithSpace(uint2str(BLspent), uint2str(DAIgot) ));\n'''
        return action
    ...
    def add1PointValue(cls, inputs, values):
        ...
        if inputs[-1] in cls.points:
            return -2

        cls.points.append(inputs[-1])   # DAI borrowed proposed

        v0, v1 = cls.aliquotValues(values)
        cls.values[0].append(v0)  # DAI Borrowed actually
        cls.values[1].append(v1)  # BL spent
...
# Test for initial pass of data collecton: 
    WarpAction.initialPass()

    # WarpAction.runinitialPass()
